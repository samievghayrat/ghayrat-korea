...
export default function CatalogCarDetailClient({ initialCar = null }: { initialCar?: CarListing | null }) {
  const params = useParams();
  const searchParams = useSearchParams();
  const id = params.id as string;

  const { t, lang } = useApp();
  const [car, setCar] = useState<CarListing | null>(initialCar);
  const [apiLoaded, setApiLoaded] = useState(Boolean(initialCar));
  const [listingRefreshed, setListingRefreshed] = useState(false);
  ...
  useEffect(() => {
    const sessionCar = initialCar || getSessionCar(id);
    if (!initialCar && sessionCar) {
      setCar(sessionCar);
      setLoading(false);
    }
    const controller = new AbortController();
    fetch(`/api/cars/${id}`, { signal: controller.signal })
      .then(res => {
      ...
      .then(data => {
        ...
        fetch(`/api/cars/${id}/pan-auto`, { signal: controller.signal })
  ...
  }, [remoteCarDetails, listingRefreshed]);
        ...
        <div className="lg:col-span-4 order-2 lg:row-start-1 lg:col-start-9 lg:row-span-2 space-y-4 lg:sticky lg:top-24">
          <div className="bg-white rounded-2xl border border-gray-200 p-5 sm:p-6 shadow-sm">
            ...
            >
              ...
              {t('contact.exactQuote')}
            </a>
            <a
              href={contactLinks.telegram}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-3 flex items-center justify-center gap-2 rounded-xl px-3 py-2 text-sm font-semibold text-[#0088cc] transition-colors hover:bg-sky-50"
            >
