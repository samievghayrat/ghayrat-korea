...
export async function generateMetadata({ params }: AuctionDetailPageProps): Promise<Metadata> {
  ...
  return getCarPreviewMetadata(`${title} ${car.year} - Аукцион`,
    `${title}, ${car.year}, ${car.mileage.toLocaleString("ru-RU")} км. Фотографии и расчёт стоимости автомобиля.`,
    getCarShareUrl('auction', id), car.image ? resolveKCarImageUrl(car.image) : undefined);
}
